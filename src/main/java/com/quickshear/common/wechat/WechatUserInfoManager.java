...
public class WechatUserInfoManager {
	...
	String openId = "";
	// 1、微信授权回调地址
	String finalUrl = "https://api.weixin.qq.com/sns/oauth2/access_token?appid=" + appId + "&secret=" + appSecret + "&code="
	...
	String finalUrl = "https://api.weixin.qq.com/cgi-bin/user/info?access_token=" + accessToken.getToken() + "&openid="
		+ openid + "&lang=zh_CN";
	String resultFromOAuth2 = "";
	resultFromOAuth2 = httpClientUtil.sendRequest(finalUrl);
	...
	String con = "{\"subscribe\":1,\"openid\":\"ok2IAw1pI1bkZv_v-AfrvztcMZKo\",\"nickname\":\"é£\",\"sex\":1,\"language\":\"zh_CN\",\"tagid_list\":[]}";
	ObjectMapper objectMapper = new ObjectMapper();
	Map<String, Object> objMap = null;
	try {
	    objMap = objectMapper.readValue(con, Map.class);
	} catch (IOException e) {
	    e.printStackTrace();
	    log.error("getWechatOpenIdByPageAccess(", e);
	}
